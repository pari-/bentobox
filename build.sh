...
BENTO_BOX_VERSION=${BENTO_BOX_VERSION:-"9.8"}
BENTO_BOX_NAME=${BENTO_BOX_NAME:-"${BENTO_BOX_PROVIDER}/${BENTO_BOX_TYPE}-${BENTO_BOX_VERSION}"}
BENTO_DIR=${BENTO_DIR:-"${HOME}/development/git/github/bento"}
...
PACKER_VERSION=${PACKER_VERSION:-"1.4.0"}
...
PACKER_OPTIONS=${PACKER_OPTIONS:-"-var mirror=${ISO_DIR}/.. -var mirror_directory="isos" -only=${BENTO_BOX_PROVIDER}-iso"}
